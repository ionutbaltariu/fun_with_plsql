...
CREATE SEQUENCE identity_cards_id_seq START WITH 1 NOCACHE ORDER;
...
CREATE TABLE clients (
    ...
    photo             BLOB,
    identity_cards_id INTEGER NOT NULL
)
LOGGING;
...
ALTER TABLE clients ADD CONSTRAINT c_identity_card_unique UNIQUE ( identity_cards_id );
...
COMMENT ON TABLE computer_configurations IS
    'Computer configurations.';

ALTER TABLE computer_configurations ADD CONSTRAINT computer_configuration_pk PRIMARY KEY ( computer_cfg_id_pk );
...
ALTER TABLE identity_cards ADD CONSTRAINT identity_cards_pk PRIMARY KEY ( id );
...
ALTER TABLE computers
    ADD CONSTRAINT computer_configuration_fk FOREIGN KEY ( computer_cfg_id_pk )
        REFERENCES computer_configurations ( computer_cfg_id_pk )
    NOT DEFERRABLE;
...
ALTER TABLE clients
    ADD CONSTRAINT identity_card_fk FOREIGN KEY ( identity_cards_id )
        REFERENCES identity_cards ( id )
            ON DELETE CASCADE
    NOT DEFERRABLE;
...
CREATE OR REPLACE TRIGGER computer_cfg_trg BEFORE
    INSERT ON computer_configurations
    FOR EACH ROW
...
CREATE OR REPLACE TRIGGER identity_cards_id_trg BEFORE
    INSERT ON identity_cards
    FOR EACH ROW
    WHEN ( new.id IS NULL )
BEGIN
    :new.id := identity_cards_id_seq.nextval;
END;
/
